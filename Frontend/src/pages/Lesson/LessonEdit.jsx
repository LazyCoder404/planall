...
const LessonAdd = () => {
    ...
    const saveData = async (event) => {
        if (!user.grade || !user.subject || !user.date || !user.topic || !user.lesson || !user.foucus || !user.material || !user.objective || 
            !user.structure || !user.assesment) {
            setValidate(true)
        }else {
            ...
            const response = await API.post(`/lesson/edit/${_id}`, Form)
            ...
            <Layout sidebar={true}>
                <div div className="page-heading">
                    ...
                    <Breadcrumb className="d-none d-sm-none d-md-none d-lg-block">
                        ...
                        <Breadcrumb.Item active>Lesson Edit</Breadcrumb.Item>
                    </Breadcrumb>
                </div>
                        ...
                        <Card.Body>
                            <Row>
                                <Col md={4}>
                                    ...
                                    <Form.Control type="text" className="my-2" value={user.grade} onChange={onChange} name='grade' />
                                </Col>
                                <Col md={4}>
                                    <Form.Label htmlFor="Subject">Subject</Form.Label>
                                    <Form.Control type="text" className="my-2" value={user.subject} onChange={onChange} name='subject' />
                                </Col>
                                <Col md={4}>
                                    <Form.Label htmlFor="Date">Date</Form.Label>
                                    <Form.Control type="date" className="my-2" value={user.date} onChange={onChange} name='date' />
                                </Col>
                            </Row>
                            <Row>
                                <Col md={6}>
                                    <Form.Label htmlFor="Topic">Topic</Form.Label>
                                    <Form.Control type="text" className="my-2" value={user.topic} onChange={onChange} name='topic' />
                                </Col>
                                <Col md={6}>
                                    <Form.Label htmlFor="Lesson">Lesson</Form.Label>
                                    <Form.Control type="text" className="my-2" value={user.lesson} onChange={onChange}  name='lesson' />
                                </Col>
                            </Row>
                            <Row>
                                <Col lg={12} sm={12}>
