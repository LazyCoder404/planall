...
const StudentEdit = () => {
    ...
    const getData = async(id) => {
        ...
        const section = await API.post("/section",{class_id:studentData.data.data.class_id})
    ...
    const saveData = async (event) => {
        if (user.firstName === "" || user.middleName==="" || user.lastName==="" || user.rollNo==="" || user.phone==="" 
        || user.dob==="" || user.bloodGroup==="" || user.fatherName==="" || user.motherName==="" || user.address==="" 
        || user.gender==="" || user.class_id==="" || user.section_id==="" || user.parent_login_id==="" || user.parent_pass===""
        || user.flag==="" || user.Email==="" || user.Password==="") {
            setValidate(true)
        }else {
            const Form = new FormData()
            Form.append('firstName', user.firstName)
            Form.append('middleName', user.middleName)
            Form.append('lastName', user.lastName)
            Form.append('rollNo', user.rollNo)
            Form.append('phone', user.phone)
            Form.append('dob', user.dob)
            Form.append('bloodGroup', user.bloodGroup)
            ...
            Form.append('class_id', user.class_id)
            Form.append('section_id', user.section_id)
            Form.append('parent_login_id', user.parent_login_id)
            Form.append('parent_pass', user.parent_pass)
            Form.append('flag', user.flag)
            Form.append('email', user.Email)
            Form.append('password', user.Password)
            Form.append('image', selectedImage?selectedImage:user)
                        ...
                        <Card.Body>
                            ...
                            <Row>
                                ...
                                <Col md={4}>
                                    ...
                                    <Form.Control type="date" placeholder="DD-MM-YYYY" value={user.dob} className="form-control my-1" name="dob" onChange={onChange} required/>
                                </Col>
                            </Row>
                            <Row>
                                <Col md={4}>
                                    ...
                                    <Form.Select name="class_id" onChange={onChange} required>
                                        <option value="">Select Class</option>
                                        {
                                            classData.map((dataVal,ind)=>{
                                                return(
                                                    <option selected={dataVal._id===user.class_id?true:false} value={dataVal._id} key={ind}>{dataVal.class_name}</option>
                                                )
                                            })
                                        }
                                    </Form.Select>
                                    ...
                                    <Form.Select name="section_id" onChange={onChange} required>
                                        <option value="">Select Section</option>
                                        {
                                            secData.map((dataVal,ind)=>{
                                                return(
                                                    <option selected={dataVal._id===user.section_id?true:false} value={dataVal._id} key={ind}>{dataVal.section_name}</option>
                                                )
                                            })
                                        }
                                    </Form.Select>
